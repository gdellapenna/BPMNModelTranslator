...
class dmn_dtable_StagePatientDT_arguments{public Object something;
}
...
Object something = args.something;

if (BPMNExecTypeUtils.toboolean(something).equals(false)) { return new dmn_dtable_StagePatientDT_result(/*stage*/4.0);} else if (BPMNExecTypeUtils.toboolean(something).equals(false)) { return new dmn_dtable_StagePatientDT_result(/*stage*/3.0);} else if (BPMNExecTypeUtils.toboolean(something).equals(false)) { return new dmn_dtable_StagePatientDT_result(/*stage*/2.0);} else  { return new dmn_dtable_StagePatientDT_result(/*stage*/1.0);}
}
}
...
class dmn_dtable_InterpretSpiroResultsDT_arguments{public Object something;
}
...
Object something = args.something;
 ...
 { return new dmn_dtable_InterpretSpiroResultsDT_result(/*copd*/true);}
}
}
/*
 * ****************************** Process Code *************************
...
// WRITTEN: 
private Object paO2CO2=null;
private final java.util.ArrayDeque<Object> paO2CO2_stream=new java.util.ArrayDeque<>();
public Object getPaO2CO2(BPMNExecProcessUtils.ProcessStatus s, boolean readNext) {Object current = this.paO2CO2;if (readNext && !this.paO2CO2_stream.isEmpty()) {this.paO2CO2 = this.paO2CO2_stream.pop();BPMNExecProcessUtils.debugOutput(s,"	 READING next input value for paO2CO2");}return current;}
...
// READ: Gateway_0xkk1o6, Activity_0f1noex
// WRITTEN: 
...
//[outgoing edge] Activity_0f1noex - Evaluate request
...
if (BPMNExecTypeUtils.equals(getPatientAlreadyStaged(s,false),false)){//[outgoing edge] Activity_0ck2kiz - Examine and Interview patient
...
} else if (BPMNExecTypeUtils.equals(getPatientAlreadyStaged(s,false),true)){//[outgoing edge] Activity_0scqf41 - Evaluate hospitalization
...
if (BPMNExecTypeUtils.equals(getSpirometryCOPDSuggestive(s,false),true)){//[outgoing edge] Activity_182mdyp - Conduct global spirometry
...
} else if (BPMNExecTypeUtils.equals(getSpirometryCOPDSuggestive(s,false),false)){//[outgoing edge] Gateway_06mnf24
...
//[outgoing edge] Activity_19eeyaj - Plan an examination
...
if (BPMNExecTypeUtils.equals(getPatientRequireHospitalization(s,false),false)){//[outgoing edge] Activity_0xf0odk - Prescribe therapy
...
} else if (BPMNExecTypeUtils.equals(getPatientRequireHospitalization(s,false),true)){//[outgoing edge] Activity_1pl6nde - Treat exacerbations
...
if (BPMNExecTypeUtils.equals(getPaO2CO2(s,false),true)){//[outgoing edge] Activity_1x6l1dw - Prescribe oxygen therapy
...
} else if (BPMNExecTypeUtils.equals(getPaO2CO2(s,false),false)){//[outgoing edge] Gateway_07g2otl
...
if (BPMNExecTypeUtils.equals(getPatientSmoker(s,false),true)){//[outgoing edge] Activity_0a2awzt - Smoking tests
...
} else if (BPMNExecTypeUtils.equals(getPatientSmoker(s,false),false)){//[outgoing edge] Gateway_197n966
...
if (BPMNExecTypeUtils.equals(getCOPDDiagnosed(s,false),false)){//[outgoing edge] Gateway_06mnf24
BPMNExecProcessUtils.logTransition("Gateway_0y30j11","Gateway_06mnf24");
GATEWAY_Gateway_06mnf24(s.withCurrent("Gateway_0y30j11"));
} else if (BPMNExecTypeUtils.equals(getCOPDDiagnosed(s,false),true)){//[outgoing edge] Gateway_1ik5zts
...
if (BPMNExecTypeUtils.equals(getCOPDSuspected(s,false),true)){//[outgoing edge] Activity_0wlmua1 - Conduct simple spirometry
...
} else if (BPMNExecTypeUtils.equals(getCOPDSuspected(s,false),false)){//[outgoing edge] Gateway_06mnf24
...
if (BPMNExecTypeUtils.toboolean(BPMNExecTypeUtils.tonumber(getStage(s,false)) >= BPMNExecTypeUtils.tonumber(1.0)) && BPMNExecTypeUtils.toboolean(BPMNExecTypeUtils.tonumber(getStage(s,false)) <= BPMNExecTypeUtils.tonumber(2.0))){//[outgoing edge] Gateway_1lnx0yn
BPMNExecProcessUtils.logTransition("Gateway_1vbgzvs","Gateway_1lnx0yn");
GATEWAY_Gateway_1lnx0yn(s.withCurrent("Gateway_1vbgzvs"));
} else if (BPMNExecTypeUtils.toboolean(BPMNExecTypeUtils.tonumber(getStage(s,false)) >= BPMNExecTypeUtils.tonumber(3.0)) && BPMNExecTypeUtils.toboolean(BPMNExecTypeUtils.tonumber(getStage(s,false)) <= BPMNExecTypeUtils.tonumber(4.0))){//[outgoing edge] Activity_0b5bgl9 - Conduct BGA
...
//[outgoing edge] Activity_0jrklqn - Test bronchodilator reversibility
...
//[outgoing edge] Gateway_0vpt9p6 - PaO2<60mmHg OR PaCO2>45mmHg?
...
//[outgoing edge] Activity_1by7ohz - Make working diagnosis
...
//[outgoing edge] Gateway_04ijm8v - Is the patient already staged?
...
//[outgoing edge] Activity_1gs1u22 - Stage patient
...
//[outgoing edge] Gateway_0ue46bu - Does patient require hospitalization?
...
//[outgoing edge] Gateway_065vrg8 - Spirometry suggestive of COPD?
...
//[outgoing edge] Activity_0vfbgoy - Interpret results
...
//[outgoing edge] Gateway_0y30j11 - COPD diagnosed?
...
//[outgoing edge] Activity_16mwits - Make diagnosis
...
public void TASK_Activity_19eeyaj_Plan_an_examination(BPMNExecProcessUtils.ProcessStatus s) {//User Task Plan an examination [Activity_19eeyaj]
...
//[outgoing edge] Gateway_1ifzb7i - COPD suspected?
...
if (this.paO2CO2_stream.isEmpty()) java.util.Arrays.stream(BPMNExecProcessUtils.inputs.getProperty("paO2CO2", "").split(",")).forEach(i->this.paO2CO2_stream.addLast(i));
BPMNExecProcessUtils.logInput("paO2CO2",this.paO2CO2_stream);
this.paO2CO2 = this.paO2CO2_stream.pop();
...
}public void execute(Object[] _patientSmoker_stream,Object[] _patientAlreadyStaged_stream,Object[] _patientRequireHospitalization_stream,Object[] _COPDSuspected_stream,Object[] _spirometryReport_stream,Object[] _analyzedResults_stream,Object[] _COPDDiagnosed_stream,Object[] _paO2CO2_stream) {if (_patientSmoker_stream != null)java.util.Arrays.stream(_patientSmoker_stream).forEach(i->this.patientSmoker_stream.addLast(i));
if (_patientAlreadyStaged_stream != null)java.util.Arrays.stream(_patientAlreadyStaged_stream).forEach(i->this.patientAlreadyStaged_stream.addLast(i));
...
if (_paO2CO2_stream != null)java.util.Arrays.stream(_paO2CO2_stream).forEach(i->this.paO2CO2_stream.addLast(i));
...
process.execute(null/*patientSmoker*/,null/*patientAlreadyStaged*/,null/*patientRequireHospitalization*/,null/*COPDSuspected*/,null/*spirometryReport*/,null/*analyzedResults*/,null/*COPDDiagnosed*/,null/*paO2CO2*/);}}
