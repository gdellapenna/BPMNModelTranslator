...
if ((TypeUtils.tonumber(Length)>0.0 && TypeUtils.tonumber(Length)<=1.0) && (TypeUtils.tonumber(Weight)>0.0 && TypeUtils.tonumber(Weight)<=5.0)) { return new dmn_dtable_DetermineModeDT_result(/*Mode*/"car");} else if ((TypeUtils.tonumber(Length)>1.0 && TypeUtils.tonumber(Length)<=2.0) && (TypeUtils.tonumber(Weight)>0.0 && TypeUtils.tonumber(Weight)<=5.0)) { return new dmn_dtable_DetermineModeDT_result(/*Mode*/"truck");} else if ((TypeUtils.tonumber(Weight)>5.0 && TypeUtils.tonumber(Weight)<=10.0)) { return new dmn_dtable_DetermineModeDT_result(/*Mode*/"truck");} else  { return new dmn_dtable_DetermineModeDT_result(/*Mode*/"undef");}
}
}/*
 * ****************************** DMN Generated Code *************************
 */
...
public void flow_sign_declaration() {task_generic_sign_declaration();
ProcessUtils.debugOutput("JOINING FLOW flow_Gateway_07f90ke");
flow_Gateway_07f90ke();
}

public void flow_fetch_declaration() {task_generic_fetch_declaration();
ProcessUtils.debugOutput("JOINING FLOW flow_Gateway_07f90ke");
flow_Gateway_07f90ke();
}
...
if (pLength.equals(-(TypeUtils.tonumber(1.0)))){ProcessUtils.debugOutput("JOINING FLOW flow_undefined_length");
flow_undefined_length();
} else {ProcessUtils.debugOutput("JOINING FLOW flow_measure_weight");
flow_measure_weight();
}
}
...
if (consent.equals("com")){ProcessUtils.debugOutput("JOINING FLOW flow_sign_declaration");
flow_sign_declaration();
} else if (consent.equals("owner")){ProcessUtils.debugOutput("JOINING FLOW flow_fetch_declaration");
flow_fetch_declaration();
} else if (consent.equals("none")){ProcessUtils.debugOutput("JOINING FLOW flow_Gateway_07f90ke");
flow_Gateway_07f90ke();
...
public void flow_measure_weight() {task_user_measure_weight();
//exclusive gateway
ProcessUtils.debugOutput("EXCLUSIVE GATEWAY Gateway_1tgxmu2");
if (TypeUtils.tonumber(pWeight) > TypeUtils.tonumber(10.0)){ProcessUtils.debugOutput("JOINING FLOW flow_unsuppoted_weight");
flow_unsuppoted_weight();
} else {ProcessUtils.debugOutput("JOINING FLOW flow_determine_mode");
flow_determine_mode();
}
}
...
if (sMode.equals("undef")){ProcessUtils.debugOutput("JOINING FLOW flow_no_shipment");
flow_no_shipment();
} else {ProcessUtils.debugOutput("JOINING FLOW flow_choose_consent");
flow_choose_consent();
}
}
...
if (this.input_PackageWeight==null) input_PackageWeight=ProcessUtils.inputs.getProperty("input_PackageWeight", null);
if (this.input_PackageType==null) input_PackageType=ProcessUtils.inputs.getProperty("input_PackageType", null);
...
ProcessUtils.start();bpmn_process_Shipment process = new bpmn_process_Shipment();
process.init();
