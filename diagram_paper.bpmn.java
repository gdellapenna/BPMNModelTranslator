...
//Process Dynamics
    public void GATEWAY_Gateway_1tgxmu2() {//exclusive gateway
        ProcessUtils.debugOutput("EXCLUSIVE GATEWAY Gateway_1tgxmu2");
        if (TypeUtils.tonumber(pWeight) > TypeUtils.tonumber(10.0)) {
            ProcessUtils.debugOutput("JOINING FLOW EVENT_unsuppoted_weight");
            EVENT_unsuppoted_weight();
        } else {
            ProcessUtils.debugOutput("JOINING FLOW TASK_determine_mode");
            TASK_determine_mode();
        }
    }

    public void GATEWAY_Gateway_1ocbjca() {//exclusive gateway
        ProcessUtils.debugOutput("EXCLUSIVE GATEWAY Gateway_1ocbjca");
        if (sMode.equals("undef")) {
            ProcessUtils.debugOutput("JOINING FLOW EVENT_no_shipment");
            EVENT_no_shipment();
        } else {
            ProcessUtils.debugOutput("JOINING FLOW TASK_choose_consent");
            TASK_choose_consent();
        }
    }

    public void GATEWAY_Gateway_07f90ke() {//exclusive joining gateway
        ProcessUtils.debugOutput("JOINING FLOW EVENT_ready_for_shipment");
        EVENT_ready_for_shipment();
    }

    public void GATEWAY_Gateway_0i2yujj() {//exclusive gateway
        ProcessUtils.debugOutput("EXCLUSIVE GATEWAY Gateway_0i2yujj");
        if (pLength.equals(-(TypeUtils.tonumber(1.0)))) {
            ProcessUtils.debugOutput("JOINING FLOW EVENT_undefined_length");
            EVENT_undefined_length();
        } else {
            ProcessUtils.debugOutput("JOINING FLOW TASK_measure_weight");
            TASK_measure_weight();
        }
    }

    public void GATEWAY_Gateway_0u50uj6() {//exclusive gateway
        ProcessUtils.debugOutput("EXCLUSIVE GATEWAY Gateway_0u50uj6");
        if (consent.equals("com")) {
            ProcessUtils.debugOutput("JOINING FLOW TASK_sign_declaration");
            TASK_sign_declaration();
        } else if (consent.equals("owner")) {
            ProcessUtils.debugOutput("JOINING FLOW TASK_fetch_declaration");
            TASK_fetch_declaration();
        } else if (consent.equals("none")) {
            ProcessUtils.debugOutput("JOINING FLOW GATEWAY_Gateway_07f90ke");
            GATEWAY_Gateway_07f90ke();
        } else {
    ...
    public void EVENT_package_received() {//start event: package received
        ...
        ProcessUtils.debugOutput("JOINING FLOW TASK_get_length");
        TASK_get_length();
    }

    public void EVENT_unsuppoted_weight() {//end event: unsuppoted weight
        ProcessUtils.error("Unsupported Weight", 2);
    }

    public void EVENT_ready_for_shipment() {//end event: ready for shipment
        ProcessUtils.success();
    }

    public void EVENT_undefined_length() {//end event: undefined length
        ProcessUtils.error("Undefined Length", 1);
    }

    public void EVENT_no_shipment() {//end event: no shipment
        ProcessUtils.error("No Shipment", 3);
    }

    public void TASK_determine_mode() {//business rule task: determine mode
        ...
        ProcessUtils.debugOutput("JOINING FLOW GATEWAY_Gateway_1ocbjca");
        GATEWAY_Gateway_1ocbjca();
    }

    public void TASK_get_length() {//business rule task: get length
        ...
        ProcessUtils.debugOutput("JOINING FLOW GATEWAY_Gateway_0i2yujj");
        GATEWAY_Gateway_0i2yujj();
    }

    public void TASK_measure_weight() {//user task: measure weight
        ...
        ProcessUtils.debugOutput("JOINING FLOW GATEWAY_Gateway_1tgxmu2");
        GATEWAY_Gateway_1tgxmu2();
    }

    public void TASK_sign_declaration() {//generic task: sign declaration
        ProcessUtils.debugOutput("TASK sign declaration");
        ProcessUtils.debugOutput("JOINING FLOW GATEWAY_Gateway_07f90ke");
        GATEWAY_Gateway_07f90ke();
    }

    public void TASK_fetch_declaration() {//generic task: fetch declaration
        ProcessUtils.debugOutput("TASK fetch declaration");
        ProcessUtils.debugOutput("JOINING FLOW GATEWAY_Gateway_07f90ke");
        GATEWAY_Gateway_07f90ke();
    }

    public void TASK_choose_consent() {//business rule task: choose consent
        ...
        ProcessUtils.debugOutput("JOINING FLOW GATEWAY_Gateway_0u50uj6");
        GATEWAY_Gateway_0u50uj6();
    }
    ...
    public static void main(String[] args) {
        ...
        process.init();
        process.EVENT_package_received();
    }
}
