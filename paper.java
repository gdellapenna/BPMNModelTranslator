...
public void flow_Gateway_07f90ke() {flow_ready_for_shipment();
}
...
if (pLength.equals(-(TypeUtils.tonumber(1.0)))){flow_undefined_length();
} else {flow_measure_weight();
};
}
...
if (consent.equals("com")){flow_sign_declaration();
} else if (consent.equals("owner")){flow_fetch_declaration();
} else if (consent.equals("none")){flow_Gateway_07f90ke();
} else {ProcessUtils.NoDefaultError();
};
}

public void flow_measure_weight() {task_user_measure_weight();
if (TypeUtils.tonumber(pWeight) > TypeUtils.tonumber(10.0)){flow_unsuppoted_weight();
} else {flow_determine_mode();
};
}
...
if (sMode.equals("undef")){flow_no_shipment();
} else {flow_choose_consent();
};
}
